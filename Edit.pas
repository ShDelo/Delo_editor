...
begin
 if TsSpeedButton(Sender).Name = 'bntBannerRightDelete' then editBannerRight.Text := '';
 if TsSpeedButton(Sender).Name = 'btnBannerMainDelete' then editBannerMain.Text := '';
 if TsSpeedButton(Sender).Name = 'bntReklamaTextDelete' then editReklamaText.Text := '';
 if TsSpeedButton(Sender).Name = 'btnReklamaSite' then editReklamaSite.Text := '';
 if TsSpeedButton(Sender).Name = 'btnReklamaDocDelete' then editReklamaDoc.Text := ''; 
 if TsSpeedButton(Sender).Name = 'btnBannerMainRubrDelete' then editBannerMainRubr.Text := '';
end;
...
begin
 if TsSpeedButton(Sender).Name = 'btnReklamaDocBrowse' then
  OpenDialog1.Filter := 'Файл Microsoft Office или изображение|*.doc;*.jpg;*.bmp' else
   OpenDialog1.Filter := 'Файл Jpg или Bmp|*.jpg;*.bmp';
 Edit := nil;
 if TsSpeedButton(Sender).Name = 'bntBannerRightBrowse' then Edit := editBannerRight;
 if TsSpeedButton(Sender).Name = 'btnBannerMainBrowse' then Edit := editBannerMain;
 if TsSpeedButton(Sender).Name = 'btnBannerMainRubrBrowse' then Edit := editBannerMainRubr;
 if TsSpeedButton(Sender).Name = 'btnReklamaDocBrowse' then Edit := editReklamaDoc;
 if OpenDialog1.Execute then if Assigned(Edit) then Edit.Text := ExtractFileName(OpenDialog1.FileName);
end;
...
begin
 if Trim(editBannerRight.Text) = '' then cbBannerRight.Checked := False;
 if Trim(editBannerMain.Text) = '' then cbBannerMain.Checked := False;
 if Trim(editReklamaText.Text) = '' then cbReklamaText.Checked := False;
 if Trim(editReklamaSite.Text) = '' then cbReklamaSite.Checked := False;
 if Trim(editReklamaDoc.Text) = '' then cbReklamaDoc.Checked := False;
 finalStr := '';
 if cbBannerRight.Checked then tmpStr := '#bannerright=1<[>' + editBannerRight.Text + '<]>$'
  else tmpStr := '#bannerright=0<[>' + editBannerRight.Text + '<]>$';
 finalStr := finalStr + tmpStr;
 if cbBannerMain.Checked then tmpStr := '#bannermain=1<[>' + editBannerMain.Text + '<]>$'
  else tmpStr := '#bannermain=0<[>' + editBannerMain.Text + '<]>$';
 finalStr := finalStr + tmpStr;
 if cbReklamaText.Checked then tmpStr := '#text=1<[>' + editReklamaText.Text + '<]>$'
  else tmpStr := '#text=0<[>' + editReklamaText.Text + '<]>$';
 finalStr := finalStr + tmpStr;
 if cbReklamaSite.Checked then tmpStr := '#site=1<[>' + editReklamaSite.Text + '<]>$'
  else tmpStr := '#site=0<[>' + editReklamaSite.Text + '<]>$';
 finalStr := finalStr + tmpStr;  
 if cbReklamaDoc.Checked then tmpStr := '#doc=1<[>' + editReklamaDoc.Text + '<]>$'
  else tmpStr := '#doc=0<[>' + editReklamaDoc.Text + '<]>$';
 finalStr := finalStr + tmpStr;
 with FormMain.IBQuery1 do begin
  Close;
...
var
 finalStr : string;
begin
 if Trim(editBannerMainRubr.Text) = '' then cbBannerMainRubr.Checked := False;
 if cbBannerMainRubr.Checked then finalStr := '#bannermainrubr=1<[>' + editBannerMainRubr.Text + '<]>$'
  else finalStr := '#bannermainrubr=0<[>' + editBannerMainRubr.Text + '<]>$';
 with FormMain.IBQuery1 do begin
  Close;
