...
begin
 ...
 try IBDatabase1.Connected := True;
  except begin
...
var
 ...
 begin
  if pos(ReklamaType,ReklamaStr) > 0 then begin
    tmp := ReklamaStr;
    delete(tmp,1,pos(ReklamaType,ReklamaStr) + (Length(ReklamaType) - 1));
    if tmp[1] = '0' then SGFirm.Cell[Col,SGFirm.LastAddedRow].Color := $008888FF;
    if tmp[1] = '1' then SGFirm.Cell[Col,SGFirm.LastAddedRow].Color := $00A6FFA6;
    delete(tmp,1,1);
...
begin
 if ClearRows then SGFirm.ClearRows;
 IBQuery1.Close;
 IBQuery1.SQL.Text := Request;;
 if IBQuery1.ParamCount > 0 then IBQuery1.Params[0].AsString := ID;
 ...
 for i := 1 to IBQuery1.RecordCount do begin
  ...
  if IBQuery1.FieldValues['REKLAMA'] <> null then begin
...
var
 ...
 begin
  if pos(ReklamaType,ReklamaStr) > 0 then begin
    tmp := ReklamaStr;
    delete(tmp,1,pos(ReklamaType,ReklamaStr) + (Length(ReklamaType) - 1));
    if tmp[1] = '0' then SGRubr.Cell[Col,SGRubr.LastAddedRow].Color := $008888FF;
    if tmp[1] = '1' then SGRubr.Cell[Col,SGRubr.LastAddedRow].Color := $00A6FFA6;
    delete(tmp,1,1);
    delete(tmp,pos('$',tmp),length(tmp));
    tmp := AnsiReplaceStr(tmp,'<[>','');
    tmp := AnsiReplaceStr(tmp,'<]>','');
...
begin
 if ClearRows then SGRubr.ClearRows;
 IBQuery1.Close;
 IBQuery1.SQL.Text := Request;
 if IBQuery1.ParamCount > 0 then IBQuery1.Params[0].AsString := ID; 
 IBQuery1.Open; IBQuery1.FetchAll;
...
begin
 SG := nil;
 if sPageControl1.ActivePageIndex = 0 then SG := SGFirm;
 if sPageControl1.ActivePageIndex = 1 then SG := SGRubr;
 if sPageControl1.ActivePageIndex = 2 then exit; 
 SG.BeginUpdate;
...
begin
 with Sender as TNextGrid do begin
  ...
  for i := 1 to RowCount do begin
   if RowVisible[i-1] then inc(n,1);
   if Odd(n) then C := $00EDE9EB {clMenuBar} else C := clWindow;
   if Name = 'SGFirm' then begin
    Cell[0,i-1].Color := C;
    if Cell[1,i-1].Color <> $008888FF then Cell[1,i-1].Color := C;
    if Trim(Cells[2,i-1]) = '' then Cell[2,i-1].Color := C;
    if Trim(Cells[3,i-1]) = '' then Cell[3,i-1].Color := C;
    if Trim(Cells[4,i-1]) = '' then Cell[4,i-1].Color := C;
    if Trim(Cells[5,i-1]) = '' then Cell[5,i-1].Color := C;
    if Trim(Cells[6,i-1]) = '' then Cell[6,i-1].Color := C;
   end;
   if Name = 'SGRubr' then begin
    Cell[0,i-1].Color := C;
    Cell[1,i-1].Color := C;
    if Trim(Cells[2,i-1]) = '' then Cell[2,i-1].Color := C;
   end;
...
var
 ...
 begin
  if pos(ReklamaType,ReklamaStr) > 0 then begin
    tmp := ReklamaStr;
    delete(tmp,1,pos(ReklamaType,ReklamaStr) + (Length(ReklamaType) - 1));
    if tmp[1] = '0' then CB.Checked := False;
    if tmp[1] = '1' then CB.Checked := True;
    delete(tmp,1,1);
    delete(tmp,pos('$',tmp),length(tmp));
    tmp := AnsiReplaceStr(tmp,'<[>','');
    tmp := AnsiReplaceStr(tmp,'<]>','');
...
begin
 if sPageControl1.ActivePageIndex = 0 then begin // ФИРМА
  if SGFirm.SelectedCount = 0 then exit;
 ...
 if sPageControl1.ActivePageIndex = 1 then begin // РУБРИКА
 ...
 if sPageControl1.ActivePageIndex = 2 then begin // РАБОТА С БАЗОЙ
  if SGDataUtils.SelectedCount = 0 then exit;
  ID := SGDataUtils.Cells[1,SGDataUtils.SelectedRow];
  if Trim(ID) = '' then exit;
  if SGDataUtils.Cells[0,SGDataUtils.SelectedRow] = 'Фирма' then isFirm := True else isFirm := False;
  IBQuery1.Close;
  if isFirm then IBQuery1.SQL.Text := 'select * from BASE where ID = :ID'
  else IBQuery1.SQL.Text := 'select * from RUBRIKATOR where ID = :ID';
  IBQuery1.Params[0].AsString := ID;
  IBQuery1.Open; IBQuery1.FetchAll;
  FormEdit.ClearEdits;
  FormEdit.lblID.Caption := IBQuery1.FieldByName('ID').AsString;
...
begin
 if NOT DirectoryExists(AppPath + 'Pic') then begin
  MessageBox(handle,'Не удалось найти директорию "Pic"','Ошибка',MB_OK or MB_ICONERROR); exit;
 end;
 if NOT DirectoryExists(AppPath + 'Doc') then begin
  MessageBox(handle,'Не удалось найти директорию "Doc"','Ошибка',MB_OK or MB_ICONERROR); exit;
 end;
 SGDataUtils.BeginUpdate;
 SGDataUtils.ClearRows; 
 for i := SGDataUtils.Columns.Count - 1 downto 0 do SGDataUtils.Columns[i].Free;
 SGDataUtils.Columns.Add(TNxTextColumn,'Раздел'); SGDataUtils.Columns[0].Position := 0; SGDataUtils.Columns[0].Width := 80;
 SGDataUtils.Columns.Add(TNxTextColumn,'ID'); SGDataUtils.Columns[1].Position := 1; SGDataUtils.Columns[1].Width := 40;
 SGDataUtils.Columns[1].Header.Glyph := NxTextColumn1.Header.Glyph; SGDataUtils.Columns[1].Header.DisplayMode := dmTextAndImage;
 SGDataUtils.Columns.Add(TNxTextColumn,'Название'); SGDataUtils.Columns[2].Position := 2; SGDataUtils.Columns[2].Width := 300;
 SGDataUtils.Columns[2].Header.Glyph := NxTextColumn2.Header.Glyph; SGDataUtils.Columns[2].Header.DisplayMode := dmTextAndImage;
 SGDataUtils.Columns.Add(TNxTextColumn,'Баннер (ротация)'); SGDataUtils.Columns[3].Position := 3; SGDataUtils.Columns[3].Width := 145;
 SGDataUtils.Columns.Add(TNxTextColumn,'Баннер (основной)'); SGDataUtils.Columns[4].Position := 4; SGDataUtils.Columns[4].Width := 145;
 SGDataUtils.Columns.Add(TNxTextColumn,'Статья'); SGDataUtils.Columns[5].Position := 5; SGDataUtils.Columns[5].Width := 145;
 gaugeProgress.Visible := True;
 CurrentProgress := 0; gaugeProgress.Progress := 0;
 MaxProgress := SGFirm.RowCount + SGRubr.RowCount;
 gaugeProgress.MaxValue := MaxProgress;
 for i := 0 to SGFirm.RowCount - 1 do begin
  ...
  ( (reklama_Doc <> '') AND (NOT FileExists(AppPath + 'Doc\' + reklama_Doc)) ) THEN begin
   SGDataUtils.AddRow;
  ...
  Inc(CurrentProgress,1); gaugeProgress.Progress := CurrentProgress; Application.ProcessMessages;
 end;
 for i := 0 to SGRubr.RowCount - 1 do begin
...
begin
 ...
 SGDataUtils.ClearRows;
 for i := SGDataUtils.Columns.Count - 1 downto 0 do SGDataUtils.Columns[i].Free;
 SGDataUtils.Columns.Add(TNxTextColumn,'ID'); SGDataUtils.Columns[0].Position := 0; SGDataUtils.Columns[0].Width := 40;
 SGDataUtils.Columns[0].Header.Glyph := NxTextColumn1.Header.Glyph; SGDataUtils.Columns[0].Header.DisplayMode := dmTextAndImage;
 SGDataUtils.Columns.Add(TNxTextColumn,'Название'); SGDataUtils.Columns[1].Position := 1; SGDataUtils.Columns[1].Width := 300;
 SGDataUtils.Columns[1].Header.Glyph := NxTextColumn2.Header.Glyph; SGDataUtils.Columns[1].Header.DisplayMode := dmTextAndImage;
 if TsSpeedButton(Sender).Name = 'btnEmailCheck' then SGDataUtils.Columns.Add(TNxTextColumn,'Email');
 if TsSpeedButton(Sender).Name = 'btnWEBCheck' then SGDataUtils.Columns.Add(TNxTextColumn,'Сайт (WEB)');
 SGDataUtils.Columns[2].Position := 2; SGDataUtils.Columns[2].Width := 300;
 IBQuery1.Close;
 if TsSpeedButton(Sender).Name = 'btnEmailCheck' then IBQuery1.SQL.Text := 'select ID,NAME,EMAIL from BASE';
 if TsSpeedButton(Sender).Name = 'btnWEBCheck' then IBQuery1.SQL.Text := 'select ID,NAME,WEB from BASE';
 IBQuery1.Open; IBQuery1.FetchAll;
 gaugeProgress.MinValue := 0;
 gaugeProgress.MaxValue := IBQuery1.RecordCount;
 gaugeProgress.Visible := IBQuery1.RecordCount > 0;
 for i := 0 to IBQuery1.RecordCount do begin
  str := IBQuery1.Fields[2].AsString;
  if Length(Trim(str)) > 0 then begin
   ...
   while pos(',',str) > 0 do begin
    ...
    if ( (TsSpeedButton(Sender).Name = 'btnEmailCheck') AND (NOT IsValidEmail(tmp)) ) then begin
     SGDataUtils.AddRow;
    ...
    if ( (TsSpeedButton(Sender).Name = 'btnWEBCheck') AND (NOT IsValidWeb(tmp)) ) then begin
     SGDataUtils.AddRow;
     SGDataUtils.Cells[0,SGDataUtils.LastAddedRow] := IBQuery1.FieldByName('ID').AsString;
     SGDataUtils.Cells[1,SGDataUtils.LastAddedRow] := IBQuery1.FieldByName('NAME').AsString;
     SGDataUtils.Cells[2,SGDataUtils.LastAddedRow] := tmp;
